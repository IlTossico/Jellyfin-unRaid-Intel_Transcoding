...
You have a Jellyfin Docker installed on your unRaid NAS/Server, HW transcoding doesn't work at all.
You have an Intel CPU with iGPU from 7/8/9th gen up. 
...
Nothing fancy to do, taking for granted you already have installed the `Community Applications` plugin on your unRaid instance, then go to "APP" and search for `Intel` on the search bar and install the `Intel-GPU-TOP` plugin from the "ich777" repository. (searching directly for Intel-GPU-TOP give me nothing)
...
chown :video /dev/dri/
```
To make sure Jellyfin have the right permission to use this device. (On some system the `video` Group can be named `render`, in case it gives you an "invalid group" error)
<br>
<br>
...
On top of that, with this, i surely resolved my transcoding issue, or mostly, i enable the ability to have the igpu and Jellyfin talk to each other, and i'm sure of that because some CLI command give me good result and because i can transcode video from my phone. But i still have problem, like with two TVs, both my 55" 1080p OLED and my 28" 720p IPS have problems playing some films or series, some media transcode fine, some give me an error and on the 55" most of the time the screen remain totally black, even so i can see the server transcoding fine in real time, or at least i see activity on the iGPU. I think the clients for WebOS, in my situation, have many problems.
<br>
<br>

So, from the "APP" centre, search for `Jellyfin` and download the repository from `linuxserver`.
On the installation of the docker, we want to add two new Variable:
...
This one is optional, for who have issue transcoding HDR content, but considering i've tried my instance of Jellyfin after adding this one too, i suggest adding it anyway. 
<br>
<br>
<br>

There is another option, for who need it, i haven't tried it, so i don't know if it works fine.
...
We can proceed with the normal setup of Jellyfin, if you have problem with this step, there are plenty of tutorial online.
So, you have Jellyfin ready and working, you have setup your media Library and what you need to have it working as you like, fine.
<br>
<br>
...
- ### **VERY IMPORTANT**
  **BOTH Intel Low-Power modes NEEDS TO BE DISABLE, OR THEY WOULD BREAK THE TRANSCODING.**

- I prefer disabling the transcoding in HEVC format, considering most devices prefer H264 and on streaming there is no much difference for bandwidth, but it's a preference.
...
<br>
  ...
  This setup would work fine for 7/8/9th Gen Intel Desktop CPU.
...
Theoretically our setup should work fine.
Before testing it directly, i would like some CLI testing, you can use those CLI line to test your actual setup, or in general to see if your Jellyfin instance is talking to your GPU.
<br>
<br>
...
Pay attention, the name of the container could be different from mine. This line make sure we have hardware acceleration working via VA-API.
<br>
<br>
<br>
```
...
If you get the same result as mine, you are done! Just do a real try, try with your smartphone or TV to play a bigger file than you can and watch first on your unRaid Dashboard for the GPU Stats; and then on your Jellyfin Control panel, click on the `i` icon to see what Jellyfin say. You should see something like this: (pardon the Italian on the Jellyfin pic)
